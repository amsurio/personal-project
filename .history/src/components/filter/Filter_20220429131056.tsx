...
const Filter: FC<FiltersType> = ({ filterByBookName, filterByPrice }) => {
    ...
    const filter = (filterField: string, filterParam: string, setFilter: (name: string) => void) => {
    ...
    return (
        <div className={style.container}>
            <div className={style.containerInner}>
                ...
                <button className={`
                         ${filteredName === 'ASC' ? style.active : style.filterBtn}`} onClick={() => filter(filteredName, 'ASC', filterByBookName)}>Алфавітом</button>
                <button className={`
