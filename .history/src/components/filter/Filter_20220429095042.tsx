...
const Filter: FC<FiltersType> = ({ filterByBookName, filterByPrice }) => {
    ...
    function filterByName(filterName: string) {
        setAlphabit(filterName)
        filterByBookName(alphabit)
        setPriceASC('')
        setPriceDESC('')
    }
    const filterByHeighPrice = () => {
        setPriceASC('ASC')
    ...
    return (
        <div className={style.container}>
            <div className={style.containerInner}>
                ...
                <button className={`
                         ${priceASC === 'ASC' ? style.active : style.filterBtn}`} onClick={filterByHeighPrice}>Меншою ціною</button>
                <button className={`
                         ${priceDESC === 'DESC' ? style.active : style.filterBtn}`} onClick={filterByLowPrice}>Більшою ціною</button>
            </div>
        </div>
    )
}
