...
import { createOrder, emptyOrders, myOrders, ordersCount } from '../../consts/profilePage';
...
const Profile = () => {
    ...
    return (
        <div className={orderLength[0] ? style.container : style.phoneContainer}>
            <h4 className={style.headText}>{myOrders}</h4>
            {orderLength[0] ? <>
                <p className={style.orderInfo}>{ordersCount} {orderLength[0]}</p>
                <div className={style.ordersBlock}>
                    {orderItems.map(order => order.map(item => <Orders key={item.id} item={item} />))}
                </div>
            </> : <EmptyBlock title={emptyOrders} subTitle={createOrder} image={'https://res.cloudinary.com/wunu/image/upload/v1652028186/personalproject/Lovepik_com-401603043-online-shopping-vector-elements_wnj9hd.png'} />}
